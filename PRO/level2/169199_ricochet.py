...
def solution(board):   
    ...
    for i in range(len(board)): 
        for j in range(len(board[0])): 
            if board[i][j] == 'R': 
                que.append([i, j, 0]) # starting point 
    visited_lst = set()
    # visited list 
    steps = [[0, -1], [0, 1], [1, 0], [-1, 0]]
    while que: 
        cry, crx, crstep = que.pop(0)
        if (cry, crx) in visited_lst: 
            continue 
        if board[cry][crx] == 'G': 
            return crstep
        visited_lst.add((cry, crx))
        
        for step in steps: 
            tmy, tmx = cry, crx
            while True: 
                nxy, nxx = tmy + step[0], tmx + step[1]
                if 0 <= nxy <= (len(board)-1) and 0 <= nxx <= (len(board[0])-1) and board[nxy][nxx] != 'D':
                    tmy, tmx = nxy, nxx 
                else: 
                    que.append([tmy, tmx, crstep+1])
                    break
